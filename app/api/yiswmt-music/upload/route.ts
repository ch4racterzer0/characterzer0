...
const COOKIE_NAME = "yiswmt-upload-auth";
...
export async function POST(req: NextRequest) {
  ...
  const key = `yiswmt-music/${safeName}`;
  ...
  try {
    ...
    console.error("yiswmt-music-upload-error", err);
    return NextResponse.json(
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    console.error("yiswmt-music-delete-error", err);
    return NextResponse.json(
      { ok: false, reason: "blob-error" },
      { status: 500 },
    );
