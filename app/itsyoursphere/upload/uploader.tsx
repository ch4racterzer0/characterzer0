...
export function Uploader() {
  ...
  const loadTracks = useCallback(async () => {
    setRefreshing(true);
    try {
      const res = await fetch("/api/itsyoursphere-music/list", { cache: "no-store" });
      if (!res.ok) {
        setTracks([]);
        return;
      }
  ...
  async function deleteTrack(url: string) {
    const u = new URL("/api/itsyoursphere-music/upload", window.location.origin);
  ...
  async function signOut() {
    await fetch("/api/itsyoursphere-music/auth", { method: "DELETE" });
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col gap-6">
      <p className="text-[10px] tracking-[0.25em] uppercase text-stone-500">
        all uploads land in{" "}
        <span className="text-stone-300">itsyoursphere-music/still-with-us/</span>{" "}
