...
async function isAuthed(): Promise<boolean> {
  const expected = process.env.YISWMT_UPLOAD_TOKEN;
  if (!expected) return false;
  const cookieStore = await cookies();
  const provided = cookieStore.get("yiswmt-upload-auth")?.value;
...
export default async function UploadPage() {
  ...
  const configured = Boolean(process.env.YISWMT_UPLOAD_TOKEN);
      ...
      <main className="min-h-screen bg-black text-stone-100 flex flex-col items-center px-4 py-10 font-mono">
        <header className="w-full max-w-2xl mb-8">
          ...
          <p className="text-xs text-stone-500 mt-2 leading-relaxed">
            files land in blob prefix <span className="text-stone-300">yiswmt-music/</span>{" "}
            and stream through the army radio on /yiswmt automatically. accepted:
        ...
        {!configured ? (
          <div className="w-full max-w-2xl border border-red-900/60 bg-red-950/30 rounded-md px-5 py-4 text-sm text-red-200">
            ...
            <p className="text-red-300/85 leading-relaxed">
              Set <code className="text-stone-200">YISWMT_UPLOAD_TOKEN</code> in
