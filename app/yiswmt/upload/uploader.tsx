...
export function Uploader() {
  ...
  const uploadOne = useCallback(
    async (item: UploadItem) => {
      ...
      try {
        const res = await fetch("/api/yiswmt-music/upload", {
          method: "POST",
          body: form,
        });
        if (res.ok) {
  ...
  async function deleteTrack(url: string) {
    const u = new URL("/api/yiswmt-music/upload", window.location.origin);
  ...
  async function signOut() {
    await fetch("/api/yiswmt-music/auth", { method: "DELETE" });
